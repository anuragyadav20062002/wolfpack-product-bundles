...
console.log(`  INNGEST_EVENT_KEY:  ${process.env.INNGEST_EVENT_KEY ? "Loaded" : "Missing"}`);
console.log(`  INNGEST_DEV:        ${process.env.INNGEST_DEV === "1" ? "1 (routing to local Dev Server at :8288)" : "unset (Inngest Cloud)"}`);
const inngestReady = !!(process.env.INNGEST_EVENT_KEY || process.env.INNGEST_DEV === "1");
console.log(`  Inngest mode:       ${inngestReady ? "Inngest queue (durable retries)" : "⚠️  Direct processing fallback (no INNGEST_EVENT_KEY or INNGEST_DEV=1)"}`);
console.log();
console.log("=".repeat(60));
console.log();
