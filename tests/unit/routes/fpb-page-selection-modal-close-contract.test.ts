...
describe("FPB page selection modal close contract", () => {
  ...
  it("uses a controlled dialog instead of the non-closing s-modal wrapper", () => {
    ...
    expect(source).not.toContain("pageSelectionModalRef");
  });

  it("wires the X button and backdrop directly to the page-selection close handler", () => {
    expect(modalSource).toContain("onClick={handlePageSelectionBackdropClick}");
