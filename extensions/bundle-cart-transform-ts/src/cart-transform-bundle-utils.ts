...
export function checkCartMeetsBundleConditions(
  ...
  const minimumQuantity = firstRule ? (firstRule.minimumQuantity || 2) : 2;
...
export function getApplicableDiscountRule(
  ...
  const applicableRules = bundleData.pricing.rules
    .filter((rule) => totalQuantity >= (rule.minimumQuantity || 0))
    .sort((a, b) => (b.minimumQuantity || 0) - (a.minimumQuantity || 0));
