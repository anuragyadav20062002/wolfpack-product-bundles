...
| 2 | PPB multipart field names | Raw product-page widget source | `items[index][id]`, `items[index][quantity]`, `items[index][properties][Box]`, `items[index][properties][_easyBundle:OfferId]`, `items[index][properties][_easyBundle:prodQty]` | EB field shape |
