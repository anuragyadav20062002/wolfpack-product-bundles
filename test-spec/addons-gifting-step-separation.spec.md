...
| 6 | Classic paid add-on card uses product add copy | Paid add-on step card rendered through shared product card in Classic | Card add button uses the configured product add label | Current EB Classic proof shows paid add-on product cards use `Add To Box`; the footer remains the cart action. |
| 6b | Classic paid add-on card renders active tier badge | Paid add-on step card rendered through shared product card in Classic with eligible `10%` tier | Card DOM includes the visible `10% off` add-on discount badge | Current EB Classic proof shows the paid add-on product card displays the tier badge. |
...
| 11 | Saved single-brace message aliases | Tier message uses `{remainingQuantity}` and `{discountValue}` | Runtime resolves aliases to active tier display values, including the discount unit for percentage tiers | WPB Admin can save single-brace aliases while EB-style defaults use `##addons...##` / `{{addons...}}`. |
