...
| 5 | Malformed Shopify settings file | Active theme exists but `settings_data.json` cannot be parsed | `appEmbedEnabled: false` | Fail closed so merchants see the enable banner instead of a false Active state |
...
| 15 | Shopify returns theme settings as URL body | `settings_data.json` body is `OnlineStoreThemeFileBodyUrl` and fetched content contains an enabled `bundle-app-embed` block | `appEmbedEnabled: true` | Prevents false disabled state when Shopify returns a short-lived file body URL for larger theme settings |
