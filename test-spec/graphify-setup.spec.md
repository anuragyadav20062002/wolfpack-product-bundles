...
| 3 | Graphify writes support files | `git status --short --ignored` | `.graphify_python`, `.graphify_root`, `manifest.json`, locks, temp files, dated backups, and cache files are ignored | Tracked `GRAPH_REPORT.md` and `graph.json` remain visible. |
...
| 6 | Existing graph contains old generated-output or no-longer-detected nodes | `npm run graphify:rebuild` | Nodes whose `source_file` is outside live graphify detection are pruned | Required because older semantic nodes are preserved across code-only rebuilds. |
...
- [ ] Existing stale source nodes are pruned from `graph.json`
