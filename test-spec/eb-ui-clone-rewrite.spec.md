...
| 4 | Visibility overview visual shell | Product Page Bundle Visibility overview | route uses custom EB-style card/grid/action markers, black Quick Setup Guide actions, and setup panels for Widget and Embed | Non-Polaris controls are allowed for exact clone work. |
| 5 | Widget targeting picker wiring | Widget display set to specific products or collections | route opens App Bridge product/collection resource pickers with `action:"select"` and writes both full selected data and compact page targets into `displayConfiguration` | Skai Lama help confirms manual product/collection override behavior. |
| 6 | Embed targeting picker wiring | Embed display set to specific products or collections | route opens the same pickers and writes the direct `upsellConfiguration.displayConfiguration` contract | Product Page Builder embed help confirms product/collection placement choices. |
| 7 | Compact visibility picker payload | Product Page Widget or Embed targets a specific product/collection from App Bridge picker | route persists compact product/collection references only and never stores full picker objects such as product descriptions or publication counts inside `bundleUpsellConfig` | Live Chrome save proved full picker objects push `bundle_ui_config` above Shopify's 64 KB metafield limit. |
