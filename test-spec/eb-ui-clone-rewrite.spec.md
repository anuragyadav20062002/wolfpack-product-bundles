...
| 36 | PPB Horizontal Slots measured geometry | `PDP_MODAL + MODAL` empty state in desktop and mobile storefront viewports | Product Page CSS uses desktop `345px` slot/action width with three `104.328px` columns and mobile `360px` slot/action width with three `110.66px` columns | EB runtime JSON provides the measured desktop/mobile targets; WPB currently renders `358-360px` wrappers on the audited fixture. |
