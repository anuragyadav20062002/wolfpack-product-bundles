    ...
    try {
      ...
      const bundleConfig = {
        ...
        steps: bundle.steps?.map((step: any) => ({
          id: step.id,
          name: step.name,
          products: componentProducts.filter(p =>
            step.StepProduct?.some((sp: any) => sp.productId === p.id)
          ),
          conditions: {
        ...
        pricing: bundle.pricing || null,
        // Add product IDs for cart transform matching
        componentProductIds: componentProducts.map(p => p.id),
        bundleProductId: productId
      };

      // Set configuration metafields
      const metafields = [
        {
          ownerId: productId,
        ...
        },
        {
          ...
          type: "json",
          value: JSON.stringify(componentProducts.map(p => {
            // Get first variant ID for each product
            return `gid://shopify/ProductVariant/${p.id.replace('gid://shopify/Product/', '')}`;
          }))
        },
        {
          ownerId: productId,
          ...
          type: "json",
          value: JSON.stringify(componentProducts.map(() => 1)) // Default quantity 1 for each
        }
      ];

      // Add price adjustment if discount is configured
      if (bundle.pricing?.enabled && bundle.pricing.rules?.length > 0) {
        ...
        } else if (bundle.pricing.method === 'fixed_amount_off') {
          priceAdjustment = {
            fixedAmountDecrease: {
            amount: (parseFloat(rule.discountValue) || 0).toFixed(2),
      ...
      const SET_METAFIELDS_MUTATION = `
        mutation SetBundleConfigMetafields($metafields: [MetafieldsSetInput!]!) {
          metafieldsSet(metafields: $metafields) {
            metafields {
              id
              key
      ...
      if (data.data?.metafieldsSet?.userErrors?.length > 0) {
        console.error('❌ [CONFIG_METAFIELDS] Configuration metafield errors:', data.data.metafieldsSet.userErrors);
        return false;
      }

      console.log(`✅ [CONFIG_METAFIELDS] Set ${data.data?.metafieldsSet?.metafields?.length || 0} configuration metafields`);
      return true;
