...
/**
 * Server-side hosting route for the product-page bundle widget
 *
 * This route serves the product-page specific widget (vertical step boxes layout).
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  try {
    // Read the product-page widget file
    const filePath = join(
      process.cwd(),
      "app",
      "assets",
    ...
    return new Response(content, {
      ...
      headers: {
        ...
        "ETag": isDevelopment
          ? `"bundle-product-page-dev-${Date.now()}"`
          : `"bundle-product-page-v1.0.0"`,
    ...
    AppLogger.error("Error serving product-page bundle widget", { component: "assets.bundle-widget-product-page", operation: "loader" }, error);

    // Return a fallback error response
    return new Response(
      `console.error('Failed to load product-page bundle widget: ${error}');`,
      {
        status: 500,
        headers: {
          "Content-Type": "application/javascript; charset=utf-8",
