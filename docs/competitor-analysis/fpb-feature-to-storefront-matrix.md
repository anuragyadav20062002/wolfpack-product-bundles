...
| ID | Group | Feature / state | EB storefront contract | ST | CL | CO | HO |
| --- | --- | --- | --- | --- | --- | --- | --- |
| N01 | F2 | No rule | Selection remains open-ended and progression follows EB | **P** F2-NO-RULE-ALL | **P** F2-NO-RULE-ALL | **P** F2-NO-RULE-ALL | **P** F2-NO-RULE-ALL |
...
| N04 | F2 | Step quantity maximum | Maximum is enforced without corrupting selected state | **P** F2-MAX-SWEEP | **P** F2-MAX-SWEEP | **P** F2-MAX-SWEEP | **P** C04 |
| N05 | F2 | Step amount rule | Undiscounted, non-default step subtotal controls eligibility | **P** F2-AMOUNT-ALL | **P** F2-AMOUNT-ALL | **P** F2-AMOUNT-ALL | **P** F2-AMOUNT-ALL |
| N06 | F2 | Step weight rule | Selected variant weight controls eligibility when current FPB Admin exposes it | **P** F2-WEIGHT-ALL | **P** F2-WEIGHT-ALL | **P** F2-WEIGHT-ALL | **P** F2-WEIGHT-ALL |
...
| N12 | F2 | Validation disabled | Saved control permits otherwise invalid progression/cart without stale errors | **P** F2-VALIDATION-OFF | **P** F2-VALIDATION-OFF | **P** F2-VALIDATION-OFF | **P** F2-VALIDATION-OFF |
...
| M03 | F4 | Product Slots disabled | Summary uses the EB text/row branch | **P** P03 | **S** | **S** F4-SLOTS-OFF | **S** F4-SLOTS-OFF |
| M04 | F4 | Product Slots enabled | Summary uses empty/filled image slots based on quantity rules | **P** P03 | **S** | **P** F4-CARD-SUMMARY | **P** F4-CARD-SUMMARY |
...
| ID | Group | Feature / state | EB storefront contract | ST | CL | CO | HO |
| --- | --- | --- | --- | --- | --- | --- | --- |
| D01 | F3 | Discounts disabled | Original totals render with no progress/message leak | **P** F3-DISABLED-ALL | **P** F3-DISABLED-ALL | **P** F3-DISABLED-ALL | **P** F3-DISABLED-ALL |
...
| `F4-SLOTS-OFF` | WPB Compact and Horizontal Product Slots-disabled replay at `1280x800` and `390x844`. Both presets used the selected-product row branch, rendered no empty slot tiles, kept the mobile tray out of slot mode, and had zero document overflow. Cells remain `S`, not `P`, until the equivalent current EB preset replays are captured. | `/private/tmp/fpb-feature-parity/F4/wpb/{CO,HO}/runtime-summary.md` |
...
| `F2-NO-RULE-ALL` | EB and WPB first-step no-rule fixtures were each mutated once and cycled through Standard, Classic, Compact, and Horizontal. Every preset advanced at `1280x800` and `390x844` without validation feedback; persisted preset identity was checked after each hard reload. WPB served widget `5.0.202`. Both fixtures were restored to Horizontal and their original quantity rules. | `/private/tmp/fpb-feature-parity/F2/no-rule/all-presets-delta.md`, `/private/tmp/fpb-feature-parity/F2/no-rule/delta.md`, `tests/unit/assets/condition-validator.test.ts`, `test-spec/condition-validator.spec.md` |
| `F2-AMOUNT-ALL` | EB and WPB first-step amount fixtures were each mutated once and cycled through Standard, Classic, Compact, and Horizontal. EB blocked at `829` and advanced at `1158`; WPB excluded its persisted default, blocked and retained `619`, and advanced at `948`. All four presets passed at `1280x800` and `390x844`. Both fixtures were restored to Horizontal and their original quantity rules, then verified from cache-bypassed runtime config. | `/private/tmp/fpb-feature-parity/F2/amount/all-presets-delta.md`, `/private/tmp/fpb-feature-parity/F2/amount/horizontal-delta.md` |
| `F2-MAX-SWEEP` | EB and WPB first-step quantity-maximum fixtures were each mutated once and replayed through the missing Standard, Classic, and Compact cells at `1280x800` and `390x844`. Two distinct shopper selections remained valid; a third distinct add was rejected without replacing or clearing either valid selection. WPB kept its persisted default outside the rule count and served widget `5.0.202`. Horizontal retains its existing direct `C04` proof. Both fixtures were restored to Horizontal and their original quantity rules, then verified from cache-bypassed runtime config. | `/private/tmp/fpb-feature-parity/F2/maximum/missing-presets-delta.md` |
...
| `F3-DISABLED-ALL` | EB and WPB retained their saved quantity tiers while the discount master switch was disabled, then cycled through all four runtime-verified presets at `1280x800` and `390x844`. Qualified selections retained the original `1687.00` EB total and `$2277.00` WPB total with no discount, progress, or success-message leak. The WPB sweep also exposed and repaired the template dialog's enclosing-form submission and Polaris focus-trap defects in `bb9c2d19`. Both fixtures were restored to enabled baseline percentage pricing and Horizontal, then cache-bypassed reloaded. | `/private/tmp/fpb-feature-parity/F3/disabled/all-presets-delta.md`, `test-spec/fpb-template-dialog-actions.spec.md` |
| `F4-CARD-SUMMARY` | EB-first geometry and WPB cache-bypassed replays closed the Compact and Horizontal product-card and Product Slots presentation slice at `1280x800` and `390x844`. Compact uses three/two intrinsic card columns; Horizontal uses two/one row cards. Both presets reuse shared desktop/mobile filled and empty slot renderers, preserve rule-driven slot counts, and remain document-contained with expanded sticky mobile summaries. The WPB fixture was restored to Horizontal. | `/private/tmp/fpb-feature-parity/F4/card-summary-compact-horizontal.md`, `tests/unit/assets/fpb-standard-mobile-summary-action.test.ts`, `test-spec/fpb-compact-horizontal-summary-slots.spec.md` |
