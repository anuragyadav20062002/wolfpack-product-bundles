...
| `PLS2-card-stress` | Mobile post-fix accepted; desktop post-fix pending | `PLF-six-product-fixture-permutation-delta.md`, `PL03-variant-row-evidence.md`, `PLS3-collection-card-density-delta.md` | Recheck widget `5.0.148` desktop rows with long titles, prices, static variant identity, and native selectors. |
| `PLS3-collection-reload` | Live collection hydration accepted; post-fix recheck pending | `PLS3-collection-reload-evidence.md`, `PLS3-collection-card-density-delta.md` | Recheck widget `5.0.148` at desktop and narrow/default/wide placements after the content-driven row fix. EB does not preserve an empty manual category, so that substate remains EB-absent. |
...
| `PLS6-other-template-smoke` | Local behavior smoke passed; live smoke pending | `PLS6-other-template-smoke-evidence.md` | Six focused suites and 50 tests pass. Product Grid, Horizontal Slots, and Vertical Slots still require desktop/mobile Chrome storefront smoke. |
