...
| VS04 live modal/filled-row re-audit | mobile modal shows only the current step title; product cards use `Added x1`; invalid Next shows a non-dismissible body-owned toast at the viewport bottom; filled rows are image/title/remove | WPB matches on `5.0.168`; mobile rows are `358x64px` with `50px` media and the remove control at right; desktop cards are five-up at about `230x330px` | fixed shared modal title/toast ownership and Vertical child order | Accepted at 360/390/768/1280/1440 with zero overflow |
