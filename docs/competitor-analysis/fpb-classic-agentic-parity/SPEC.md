...
| Current delta | `C04-slots-box-validation/delta.md` updated with live EB/WPB evidence. EB C04 was configured through Admin UI and captured on desktop/mobile; in this exact fixture EB allowed a third item and accepted checkout despite the planned `Box of 2` over-max expectation. WPB root cause for the Standard-first flash is stale full `data-bundle-config` (`bundleDesignPresetId: "STANDARD"`) followed by proxy `CLASSIC`; current source/build includes pre-render hydration and hard reload proof shows version `5.0.19`, root `fpb-preset-classic`, and only base + Classic widget stylesheets active. Classic desktop sidebar slot CSS was fixed for the actual `.bw-selected-slot*` DOM. C04 remains open because the live WPB payload still has product slots/box selection disabled, so mobile tray renders skeleton rows instead of slot rows. |
