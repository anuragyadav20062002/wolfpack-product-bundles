...
| Field | Value |
|---|---|
| Status | wpb-mobile-slot-card-proofed-validation-fixture-gated |
...
| Current delta | `C04-slots-box-validation/delta.md` updated with current EB/WPB evidence. EB Admin currently shows Product Slots enabled, a slot icon control with Change/Reset actions, and the visible quantity validation control checked. Cache-bypassed EB storefront proof still emits `validateBoxSelectionQuantity: false`, so under-min/exact-max/over-max blocking is not proved by this fixture. EB mobile expanded tray proves Classic slot rendering: one visible filled slot image (`65 x 80` image inside a `65 x 60` slot), one visible `65 x 60` dashed empty slot, active `Box of 2`, `₹5 off`, and `items_quantity: 1`; the current empty slot renders with no icon image/background. WPB was reconfigured through Chrome DevTools MCP after the dev tunnel restart: Product Slots enabled, one step/category, Discount & Pricing enabled, Rule #1 set to Fixed Amount Off with Quantity >= 2 and amount `5`, Bundle Quantity Options enabled, box label `Box of 2`, and box subtext `$5 off`. Source fixes remain storefront-scoped: `5.0.25` inserted Classic mobile box-selection rendering, and `5.0.28` reuses the existing compact mobile slot-tile renderer for slot-enabled `CLASSIC` summaries with Classic-only CSS. Cache-bypassed WPB proof on `window.__BUNDLE_WIDGET_VERSION__ === "5.0.28"` shows root `CLASSIC`, one visible category, active mobile/desktop `Box of 2` / `$5 off`, mobile expanded filled state with two `fpb-mobile-summary-slot-card` nodes, zero mobile product-row nodes, a filled `65 x 60` slot containing a `65 x 80` product image, and a `65 x 60` empty slot with `2px dashed rgb(0, 0, 0)` border. Desktop smoke still shows two sidebar slot nodes and no mobile slot-card nodes. Evidence: `wpb-admin-reconfigure-c04-20260703.response.network-response`, `wpb-c04-mobile-initial-runtime-5028-20260703.json`, `wpb-c04-mobile-filled-slot-runtime-5028-20260703.json`, `wpb-c04-mobile-filled-slot-5028-20260703.png`, `wpb-c04-desktop-initial-runtime-5028-20260703.json`, and `wpb-c04-desktop-filled-slot-runtime-5028-20260703.json/png`. Remaining C04 gaps: custom slot icon parity remains unproved because current EB proof also shows no visible custom icon image/background; validation blocking remains unverified because EB still emits `validateBoxSelectionQuantity: false`. |
