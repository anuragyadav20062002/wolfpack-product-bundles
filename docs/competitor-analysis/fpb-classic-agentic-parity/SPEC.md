...
| Current delta | WPB-only mobile proof captured on `5.0.21`: final CTA is disabled after Step 1 selection and before Step 2 selection; after Step 2 selection, `cart/add.js` posts two component lines with `_wolfpackProductBundle:OfferId` and `_bundle_display_properties`; Cart Transform produces one parent cart line with `_is_bundle_parent`, component count, retail/price/savings cents, and no discount allocation; `/apps/product-bundles/api/cart-bundle-details` receives `bundleDetailsKey: "FBP-cmr361mz50000v00yrdeyxpf7_UYM"` and display properties. Evidence: `/private/tmp/fpb-classic-agentic-parity/C08-cart-lines/wpb-cart-proof-5021.json`, `wpb-cart-add-5021.network-request`, and `wpb-cart-bundle-details-5021.network-request`. Durable EB reference confirms FPB uses JSON `POST /cart/add.js` component lines plus Storefront GraphQL `bundle_details`; current live EB bundle `2` cannot be re-proved end to end because the storefront crashes before product controls render with `Cannot read properties of null (reading 'personalizationData')`. Evidence and comparison notes are in `/private/tmp/fpb-classic-agentic-parity/C08-cart-lines/delta.md`. EB live cart proof and discount/cart-line savings comparison are still pending. |
