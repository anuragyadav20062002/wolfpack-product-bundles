...
| Current delta | `C04-slots-box-validation/delta.md` updated with the 2026-07-04 exact-2 reconfiguration. EB Admin proof shows `Pre Selected Product` disabled, Step Rule changed to exact `2`, Product Slots enabled, Slot Icon controls still present, and Bundle Quantity Options still set to `Box of 2` / `₹5 off`. Cache-bypassed EB desktop/mobile storefront proof now starts at zero selected items and renders two empty dashed Classic slots, but both `stepsConfigurationData.boxSelection.validateBoxSelectionQuantity` and `gbbBoxSelection.state.validateBoxSelectionQuantity` remain `false`; clicking next at zero selected produces no validation copy. EB empty slots still have `backgroundImage: none` and no custom icon source, so no WPB custom-icon source change is justified. Fresh WPB true-mobile proof on `window.__BUNDLE_WIDGET_VERSION__ === "5.0.32"` shows root `CLASSIC`, expanded tray `Review your bundle`, active `Box of 2` / `$5 off`, two empty slot cells, progress copy `Add 2 product(s) to save $5!`, and `Add To Cart • $0.00`. Evidence includes `eb-admin-c04-validation-reconfigured-20260704.txt`, `eb-admin-c04-discount-pricing-validation-search-20260704.txt`, `eb-c04-validation-desktop-runtime-before-next-20260704.json`, `eb-c04-validation-mobile-expanded-runtime-20260704.json`, `eb-c04-validation-mobile-zero-next-button-click-20260704.json`, and `wpb-c04-current-mobile-expanded-runtime-20260704b.json`. Remaining gap: under-min/exact-max/over-max blocking cannot be accepted until EB emits `validateBoxSelectionQuantity: true` or another verified validation-enabled storefront runtime. |
...
| WPB config | Dev-tunnel Classic storefront fixture reconfigured through Admin UI for one fixed-amount quantity tier and Bundle Quantity Options, with Progress Bar and Discount Messaging disabled; Admin flow/business logic remains locked except fixture setup |
...
| Current delta | EB bundle `1` remains the recoverable Classic storefront evidence source for the fixed-amount quantity tier. EB Admin proof shows `Fixed Amount Off`, quantity threshold `2`, fixed amount `₹5`, Bundle Quantity Options enabled with `Box of 2` / `₹5 off`, and both Progress Bar and Discount Messaging disabled. Cache-bypassed EB desktop/mobile storefront proof keeps the box selector visible, hides `gbbDiscountComponent`, and renders no discount-progress/message copy; only the step timeline progress remains visible. WPB previously ignored the disabled display toggles in shared full-page summary code: desktop still formatted side-panel discount messages, compact mobile forced progress rendering, and FPB save persisted stale nested `displayOptions.progressBar.enabled: true`. Source now respects `showDiscountMessaging`, stops compact mobile from forcing progress, and serializes canonical display options from the top-level progress checkbox. After Admin re-save on the current dev tunnel, cache-bypassed WPB proof on `window.__BUNDLE_WIDGET_VERSION__ === "5.0.33"` shows root `CLASSIC`, visible `Box of 2` / `$5 off`, no visible summary discount-message/progress nodes on desktop, no mobile collapsed discount copy, and no mobile expanded discount/progress nodes. Evidence includes `eb-admin-c05-before-progress-config-20260704.txt`, `eb-c05-post-c04-desktop-runtime-20260704.json`, `eb-c05-post-c04-mobile-expanded-runtime-20260704.json`, `wpb-admin-c05-post-fix-resave-20260704.response.network-response`, `wpb-c05-post-save-desktop-runtime-20260704.json`, `wpb-c05-post-save-mobile-collapsed-runtime-20260704.json`, and `wpb-c05-post-save-mobile-expanded-runtime-20260704.json`. Remaining C05 gaps: fixed-price tiers, buy-X-get-Y, multiple/highest-eligible discount conflicts, EB visible progress-bar-on behavior, and cart-line savings proof for the fixed-amount box rule. |
...
| Field | Value |
|---|---|
| Status | source-proofed-oos-inventory-gap |
