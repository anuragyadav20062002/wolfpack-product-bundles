...
| Field | Value |
|---|---|
| Status | two-tier-message-proofed-cart-gap |
...
| Field | Value |
|---|---|
| Status | live-cart-proofed-eb-blocked-and-graphql-gap |
...
| Current delta | EB bundle `1` provides live Classic success proof: Step 2 routes through `personalizationPage`, add-on review pricing is visible, final `Add To Cart` triggers `cart/add.js` 200, `cart/update.js` 200, and checkout. The current EB bundle `1` checkout fixture collapses paid add-on details into one parent `Daily Essentials` line, but the durable EB reference and prior paid-add-on proof in `internal docs/EB Implementation Reference.md` show the implementation target is a separate discounted add-on line plus a parent bundle line, so no WPB source change is justified to force the fixture-specific collapse. Fresh WPB `5.0.27` proof on the Classic two-tier fixture posts three `cart/add.js` lines: two bundle components and one `100%` add-on line with `_addon_product: "true"`, `_boxProduct: "addonProduct"`, `_addonTierId: "tier2"`, `_bundle_step_type: "addon:PERCENTAGE:100"`, and `_addon_offer_id`. Checkout `/cart.js` shows two lines: a free discounted `Fragrance Candle - Cherry` add-on line with `discounts: [{ title: "Add On", amount: 3000 }]` and one parent `Daily Essentials` line with `_is_bundle_parent: "true"`, `_bundle_component_count: "2"`, `_addon_offer_id`, `Box: "1"`, and bundle savings properties. The checkout summary shows `ADD ON (-$30.00)`, the free add-on line, `Hide 2 items`, subtotal `$1,443.00`, and total savings `$30.00`; mobile checkout proof at `390 x 844` confirms the same cart JSON and visible order summary. `/apps/product-bundles/api/cart-bundle-details` receives `bundleDetailsKey: "FBP-cmr361mz50000v00yrdeyxpf7_E6M"` and returns `{ ok: true }` on a repeated proof POST. Current evidence includes `wpb-c08-cart-add-5027.request.network-request`, `wpb-c08-cart-bundle-details-5027.request.network-request`, `wpb-c08-cart-bundle-details-repeat-response-5027.json`, `wpb-c08-after-add-cart-json-5027-20260703.json/png/txt`, and `wpb-c08-checkout-mobile-runtime-5027-20260703.json/png/txt`. Remaining C08 gaps: EB blocked-state proof and EB `bundle_details` before/after GraphQL proof for bundle `1`. |
