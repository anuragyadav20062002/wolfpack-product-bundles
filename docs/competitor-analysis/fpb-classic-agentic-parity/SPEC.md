...
| Field | Value |
|---|---|
| Status | wpb-mobile-box-selector-proofed-validation-fixture-gated |
...
| Current delta | `C04-slots-box-validation/delta.md` updated with current EB/WPB evidence. EB Admin currently shows Product Slots enabled, a slot icon control with Change/Reset actions, and the visible quantity validation control checked. Cache-bypassed EB storefront proof still emits `validateBoxSelectionQuantity: false`, so under-min/exact-max/over-max blocking is not proved by this fixture. EB mobile expanded tray proves Classic slot rendering: one visible filled slot image, one visible dashed empty slot, active `Box of 2`, `₹5 off`, and `items_quantity: 1`; the current empty slot renders as a dashed box with no icon image/background in this fixture. WPB was reconfigured through Chrome DevTools MCP after the dev tunnel restart: Product Slots enabled, add-ons disabled, one step/category, Discount & Pricing enabled, Rule #1 set to Fixed Amount Off with Quantity >= 2 and amount `5`, Bundle Quantity Options enabled, box label `Box of 2`, and box subtext `$5 off`. Cache-bypassed WPB proof on `window.__BUNDLE_WIDGET_VERSION__ === "5.0.25"` shows root `CLASSIC`, active mobile box selector `Box of 2` / `$5 off`, desktop sidebar box selector unchanged, visible empty Classic slot affordances, discount progress copy `Add 2 product(s) to save $5!`, and after adding one product a mobile selected product row with 65px thumbnail, quantity `x1`, remove button, and one empty slot card. Source change is Classic-owned in the mobile summary renderer and Classic mobile CSS, reusing the existing box-selection renderer instead of adding JS layout injection. Evidence: `wpb-c04-final-reconfigured-proxy-20260703.json`, `wpb-c04-mobile-box-selector-runtime-5025-20260703.json`, `wpb-c04-mobile-box-selector-5025-20260703.png`, `wpb-c04-mobile-filled-slot-dom-5025-20260703.json`, `wpb-c04-mobile-filled-slot-5025-20260703.png`, `wpb-c04-desktop-box-selector-runtime-5025-20260703.json`, and `wpb-c04-desktop-box-selector-5025-20260703.png`. Remaining C04 gaps: exact EB compact slot-card pixel parity remains open because WPB renders the selected mobile slot as a Classic summary product row rather than EB's square footer slot; custom slot icon parity remains unproved because current EB proof also shows no visible custom icon image/background; validation blocking remains unverified because EB still emits `validateBoxSelectionQuantity: false`. |
...
| Field | Value |
|---|---|
| Status | fixed-amount-box-proofed-other-tiers-gap |
| EB config | Classic percentage, fixed amount, fixed-price, and highest-eligible discount tiers with progress below threshold |
| WPB config | Dev-tunnel Classic storefront fixture reconfigured through Admin UI for one fixed-amount quantity tier plus Bundle Quantity Options; Admin flow/business logic remains locked except fixture setup |
...
| Current delta | EB bundle `1` is the current recoverable Classic storefront evidence source for this row: it renders `bundleDesignPresetId: "CLASSIC"` with box selection enabled, default `Box of 2` / `₹5 off` copy, default product selection, and add-on percentage tier messaging (`Congrats you are eligible for 10% off on Add ons`). EB Admin recheck proves the active Discount & Pricing setup is `Fixed Amount Off`, quantity threshold `2`, fixed amount `₹5`, Bundle Quantity Options enabled with `Box of 2` / `₹5 off`, Progress Bar disabled, and Discount Messaging disabled; no row-specific help/setup links were visible on that surface. Cache-bypassed EB desktop/mobile storefront proof shows the visible bar nodes are the step timeline (`gbbStepsProgressBar*`), while the discount component is hidden and no discount-progress message renders. Adding a second item moves `items_quantity` from `1` to `2`, advances to Step 2, keeps the `Box of 2` / `₹5 off` label, and shows footer total `₹1658.00` for the two selected default-product instances with no visible ₹5 footer deduction. Current EB evidence includes `eb-c05-admin-discount-pricing-current-20260703.txt`, `eb-c05-current-desktop-runtime-computed-20260703.json`, `eb-c05-current-desktop-eligible-runtime-computed-20260703.json`, `eb-c05-current-mobile-expanded-runtime-20260703.json`, and `eb-c05-current-add-second-product-action-20260703.json`. Fresh WPB proof after the C04 reconfiguration and `window.__BUNDLE_WIDGET_VERSION__ === "5.0.25"` now shows root `CLASSIC`, active `Box of 2` / `$5 off`, desktop initial copy `Add 2 product(s) to save $5`, desktop below-threshold copy `Add 1 product(s) to save $5`, desktop eligible success copy `Success! Your $5 discount has been applied to your cart.`, and desktop total moving from `$1448.00` to `$1443.00`. Mobile proof at `390 x 844` shows collapsed footer unlock copy, expanded tray active box selector, below-threshold copy after one item, and eligible expanded tray with success copy plus `Add To Cart • $1443.00`. Evidence: `wpb-c05-desktop-initial-runtime-5025-20260703.json`, `wpb-c05-desktop-below-threshold-runtime-5025-20260703.json`, `wpb-c05-desktop-eligible-runtime-5025-20260703.json`, `wpb-c05-mobile-collapsed-runtime-5025-20260703.json`, `wpb-c05-mobile-expanded-empty-runtime-5025-20260703.json`, `wpb-c05-mobile-eligible-expanded-runtime-5025-20260703.json`, screenshots/a11y with matching names, and `wpb-c05-bundle-api-9005-5025.response.network-response`. Remaining C05 gaps: fixed-price tiers, buy-X-get-Y, multiple/highest-eligible discount conflicts, EB visible progress-bar-on behavior, and cart-line savings proof for the fixed-amount box rule. No source change is justified from this pass. |
...
| Current delta | EB bundle `1` proves the Classic paid add-on step surface on desktop and mobile. Step 2 renders add-on products, collapsed mobile footer (`View Selected Products`, CTA, price, count), expanded mobile summary (`Daily kit`, `Two essentials unlock savings`, `Box of 2`, `₹5 off`, add-on eligibility message), and paid add-on selection. After clicking the first Step 2 add-on, the product button changes from `Add To Box` to quantity `1`, the footer total changes from `₹1158.00` to `₹1687.00`, and the footer count changes from `2` to `3`. Fresh EB Admin proof captures the `[Classic]` add-on help article and current setup: `Add-Ons and Gifting Step` enabled, `Add-Ons with Bundles` enabled, `Step Name`/title `Add On`, one Tier 1 product, quantity threshold `1`, `10%`, and the eligible/ineligible messaging. WPB was reconfigured through Admin for the equivalent paid tier. Fresh WPB payload on `window.__BUNDLE_WIDGET_VERSION__ === "5.0.22"` shows `FBP_SIDE_FOOTER + CLASSIC`, `personalizationData.isPersonalizationEnabled: true`, `personalizeStepText: "Add On"`, and `addonProducts.type: "MULTI_TIER"` with one tier: quantity threshold `1`, one selected add-on product, percentage discount value `10`, and matching eligible/ineligible copy. Storefront desktop proof shows the Add On timeline node, pre-Add On sidebar eligibility message, Add On step discounted pricing (`$829.00` to `$746.10`), selecting the add-on, summary count changing to `3 items`, and total changing from `$2487.00` to `$2404.10`. WPB cart proof after the paid add-on selection shows `cart/add.js` 200 with two normal component lines plus one add-on line, `/apps/product-bundles/api/cart-bundle-details` 200, checkout redirect, and checkout `/cart.js` with two rendered cart lines: one discounted add-on line (`Add On` discount `-$82.90`) and one bundle parent line with `Hide 2 items`. Source fixes in `5.0.23` keep Classic paid add-on cards on the product-add label path and make the visible mobile summary footer advance one paid step at a time instead of jumping directly to a locked add-on step. Cache-bypassed mobile proof on `5.0.23` after dev-tunnel reconfiguration shows root `CLASSIC`, no Standard root in the boot trace, visible footer navigation from Step 1 to Step 1 (Copy) to Add On, Add On discounted pricing (`$829.00` to `$746.10`), and paid add-on selection changing the visible footer from `Add To Cart • $1658.00` to `Add To Cart • $2404.10`. EB/WPB comparison then identified one remaining paid add-on card surface delta: EB shows a visible `10% off` badge on the paid add-on card, while WPB `5.0.23` only showed compare/current prices. Source fix `5.0.24` allows the existing add-on discount badge markup on Classic and adds Classic-scoped desktop/mobile badge styling. Cache-bypassed WPB desktop and mobile proof on `5.0.24` shows root `CLASSIC`, only Classic preset CSS active, the Add On step card text `10% off`, `$829.00`, `$746.10`, and one `.fpb-addon-discount-badge` with computed `position: absolute`; screenshots and a11y snapshots are saved as `wpb-c06-classic-badge-desktop-5024-20260703.png`, `wpb-c06-classic-badge-desktop-runtime-5024-20260703.json`, `wpb-c06-classic-badge-desktop-a11y-5024-20260703.txt`, `wpb-c06-classic-badge-mobile-5024-20260703.png`, `wpb-c06-classic-badge-navigation-5024-20260703.json`, and `wpb-c06-classic-badge-mobile-a11y-5024-20260703.txt`. WPB was then reconfigured through Admin for two add-on tiers: Tier 1 `10%` paid add-on at quantity `1`, and Tier 2 `100%` free-gift tier at quantity `2`, with saved single-brace message variables. Source fix `5.0.26` resolves saved aliases such as `{remainingQuantity}` and `{discountValue}` in addition to existing EB-style `##addons...##` and `{{addons...}}` tokens. Cache-bypassed desktop proof shows root `CLASSIC`, no literal placeholders, initial resolved `Add 1 more product(s) to claim 10% off on Add ons`, one-item resolved `Congrats you are eligible for 10% off on Add ons`, and two-item resolved `Congrats you are eligible for 100% off on Add ons`; evidence files include `wpb-c06-desktop-message-runtime-5026-20260703.json`, `wpb-c06-desktop-message-initial-5026-20260703.png`, `wpb-c06-desktop-message-after-one-5026-20260703.json/png/txt`, and `wpb-c06-desktop-message-after-two-5026-20260703.json/png/txt`. Mobile proof at `390 x 844` on `5.0.26` shows root `CLASSIC`, no literal placeholders, initial/one-item/two-item add-on message resolution, and a resolved `100%` tier message in the mobile summary DOM; evidence files include `wpb-c06-mobile-message-initial-5026-20260703.json/png/txt`, `wpb-c06-mobile-message-after-one-5026-20260703.json/png/txt`, and `wpb-c06-mobile-message-after-two-5026-20260703.json/png/txt`. Remaining C06 gaps: after one paid item, EB shows progress toward the next `100%` tier while WPB reports the already-eligible `10%` tier; multiple-tier conflict/display behavior and EB/WPB cart-line parity remain open because EB currently collapses the paid add-on into one parent checkout line while WPB renders a discounted add-on line plus a parent line. |
