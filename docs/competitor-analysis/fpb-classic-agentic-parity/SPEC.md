...
| Field | Value |
|---|---|
| Status | wpb-mobile-slot-card-proofed-desktop-css-proofed-validation-fixture-gated |
...
| Current delta | `C04-slots-box-validation/delta.md` updated with current EB/WPB evidence. EB Admin currently shows Product Slots enabled, a slot icon control with Change/Reset actions, and the visible quantity validation control checked. Cache-bypassed EB storefront proof still emits `validateBoxSelectionQuantity: false`, so under-min/exact-max/over-max blocking is not proved by this fixture. EB desktop proof from 2026-07-04 shows current Classic slot geometry as a `170 x 90` rail with two `80 x 80` slots and no visible custom empty-slot icon/background; EB mobile expanded tray proof shows a `65 x 80` filled image inside a `65 x 60` slot, one `65 x 60` dashed empty slot, active `Box of 2`, `₹5 off`, and `items_quantity: 1`. WPB storefront proof on `window.__BUNDLE_WIDGET_VERSION__ === "5.0.32"` shows the live bundle still serving `FBP_SIDE_FOOTER + CLASSIC`, Product Slots enabled, `Box of 2` / `$5 off`, and `validateBoxSelectionQuantity: false`; the fixture currently has the later C07 category set (`C07 Manual Products`, `C07 Collection Source`, `C07 Mixed Source`) because the Shopify Admin iframe stayed on `/auth/session-token` and did not reach the configure form after the dev-tunnel restart. No DB shortcut was used. Storefront-scoped source fixes remain: `5.0.25` inserted Classic mobile box-selection rendering, `5.0.28` reuses the compact mobile slot-tile renderer for slot-enabled `CLASSIC` summaries, and `5.0.32` changes only Classic desktop slot CSS so the generated `bundle-widget-full-page-classic.css` carries an `inline-grid` 5rem sidebar rail matching the current EB `80 x 80` desktop slot size. Current WPB mobile expanded proof at the available MCP viewport (`500 x 725`) shows root `CLASSIC`, active `Box of 2` / `$5 off`, one filled `65 x 60` mobile slot with a `65 x 80` image, and one `65 x 60` dashed empty slot. Evidence: `eb-c04-current-desktop-runtime-20260704.json`, `eb-c04-current-desktop-20260704.png`, `wpb-c04-5032-current-mobile-expanded-runtime-20260704.json`, `wpb-c04-5032-current-mobile-expanded-20260704.png`, `reconfigure-check-bundle-api-20260704.json`, and `admin-auth-stuck-20260704.json`. Remaining C04 gaps: true fresh `390 x 844` mobile proof, live Admin reconfiguration back to the one-category C04 fixture, custom slot icon parity, and validation blocking remain unverified. |
