...
- Current WPB dev preview cannot be used for final storefront parity proof until the parity fixture bundle is recreated or restored in the current SIT DB and then saved/synced through Admin. Source commits `8670ae11` and `b9ee2323` restore both full-page cache consumers: the widget now uses a full `data-bundle-config` payload without proxy fetch, and generated Shopify page markers include the formatted bundle config when the server refresh path has bundle data. The follow-up session-storage source fix retries stale Prisma closed-connection reads once, but it correctly does not mask persistent database reachability failures. Fresh Chrome DevTools MCP retries on 2026-07-02 no longer surface the stale-Prisma text in the captured storefront path; instead the current SIT dashboard is empty, the old bundle id renders the app's 404, and the storefront still serves only the stale bootstrap marker on widget `5.0.1`.
