...
- Chrome DevTools MCP asset probe on 2026-07-02 confirmed the remaining `5.0.4` live-proof gap is the Shopify extension CDN asset itself, not browser Cache Storage. Evidence file `/private/tmp/fpb-standard-agentic-parity/final-wrap-current-wpb/wpb-asset-version-probe.json` shows the storefront loading `https://cdn.shopify.com/extensions/.../assets/bundle-widget-full-page-bundled.js` with `cache-control: public, max-age=31557600`; the fetched script body starts with `Version : 5.0.3`, contains `window.__BUNDLE_WIDGET_VERSION__ = '5.0.3'`, and does not contain `5.0.4`.
...
- Remaining smoke gap: source isolation note `regression-smoke/delta.md` could not be written in this session because `/private/tmp` writes requiring approval were blocked by the environment usage-limit gate. Runtime proof still confirms Standard selectors did not apply to Classic, Compact, or Horizontal.
...
- Regression smoke runtime/cart proof is complete for the current replacement fixture across Standard, Classic, Compact, and Horizontal on widget `5.0.3`, including Standard cart-add proof and Horizontal non-Standard cart-add proof. The source-isolation delta note remains the only smoke artifact gap and does not replace the row-specific mirrored EB/WPB fixtures still listed above.
