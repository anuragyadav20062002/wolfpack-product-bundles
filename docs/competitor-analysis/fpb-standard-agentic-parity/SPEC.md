...
- Remaining P09 gap: live WPB checkout still needs SIT deploy/re-auth so Shopify has the new discount extension, `read_discounts` scope, automatic app discount activation, and Cart Transform update. After deploy, recapture `/cart.js` and checkout proof for original price, discounted price, and native `Add On` allocation.
