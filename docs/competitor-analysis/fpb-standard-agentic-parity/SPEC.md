...
- Current WPB dev preview cannot be used for final storefront parity proof until the page marker is refreshed and the app-proxy/DB path recovers. Source commits `8670ae11` and `b9ee2323` restore both full-page cache consumers: the widget now uses a full `data-bundle-config` payload without proxy fetch, and generated Shopify page markers include the formatted bundle config when the server refresh path has bundle data. The follow-up session-storage source fix retries stale Prisma closed-connection reads once, but it correctly does not mask persistent database reachability failures. A fresh Chrome DevTools MCP retry on 2026-07-02 still shows the Admin app iframe returning `500` and the storefront serving only the stale bootstrap marker on widget `5.0.1`.
