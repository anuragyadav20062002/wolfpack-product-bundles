...
- Remaining P10 gap: a desktop no-duplicate storefront/cart/checkout recapture for the saved multi-tier free-add-on fixture is still pending. After the checkout-extension source build, the storefront started loading Shopify CDN dev theme asset `bundle-widget-full-page-bundled.js` at `404`, leaving the widget root hydrated but empty; proof is `wpb-desktop-nodup-load-probe.json`, `wpb-desktop-nodup-resource-probe.json`, `wpb-desktop-nodup-widget-script-audit.json`, and `wpb-desktop-nodup-load-timeout.snapshot.txt`. Do not mark P10 fully verified until a desktop `1280+` no-duplicate pass confirms the separate free add-on line matches the existing mobile proof.
...
| P10 | partial-live-proof | `/private/tmp/fpb-standard-agentic-parity/P10-free-addon-highest-tier/`; `/private/tmp/fpb-standard-agentic-parity/P10-free-addon-highest-tier/current-wpb/` |
