...
- Remaining P10 gap: desktop storefront/cart/checkout recapture for the saved multi-tier free-add-on fixture is still pending. The current Chrome session stayed in mobile emulation after `resize_page` and local Chrome window restore attempts; proof is `wpb-desktop-viewport-probe.json` with `innerWidth: 390` and `devicePixelRatio: 3`. Do not mark P10 fully verified until a desktop `1280+` pass is captured.
