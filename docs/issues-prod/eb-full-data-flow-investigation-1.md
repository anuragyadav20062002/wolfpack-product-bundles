...
- Captured FPB `POST /cart/add.js` payload: component items with `_wolfpackProductBundle:OfferId: "FBP-2_K6C_1"`, followed by `GetCartMetafield` + `cartMetafieldsSet` Storefront API sequence writing `bundle_details` metafield.
...
- Captured PPB `POST /cart/add` payload (multipart/form-data): component items with `_wolfpackProductBundle:OfferId: "MIX-894502_K1K_1"`. Cart Transform OVERWRITE_LINE_ITEM operation converts component item to parent bundle product in response.
