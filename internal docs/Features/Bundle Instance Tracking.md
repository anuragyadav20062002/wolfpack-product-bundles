...
This ID is added as a **cart attribute** (`_bundle_id`) to every component line item in the same bundle add operation.

## Why It Works

- Shopify's `/cart/add.js` does NOT merge lines that have different `properties`/attributes
- Cart Transform groups component lines by matching `_bundle_id` attribute for MERGE operations
- Two adds of the same bundle → two different `_bundle_id` values → two separate merged lines
