...
- Current Classic mobile footer proof on 2026-07-06 shows a transient `Additional offers to be unlocked` state in the centered footer count pill. It appears after selecting one paid product when one add-on tier is eligible and another higher tier remains locked. The pill stays black with count `1` for roughly the first half second, turns green (`rgb(85, 189, 71)`) while still showing the count, swaps to `Additional offers to be unlocked` around `800ms`, stays visible until roughly `2.5s`, and returns to the normal black numeric count before `3.2s`. It does not appear at zero selected products, on the add-on step itself, or after all add-on tiers are eligible.
