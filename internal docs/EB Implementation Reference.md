...
2026-07-06 Classic sidebar add-on scrollbar proof: selecting more than two add-on products in the live EB Classic add-on step does not create a selected-add-ons list in the desktop summary sidebar; the sidebar switches to the normal selected bundle summary, total, and cart action. The add-on tier/message stack visible before the add-on step is a bounded vertical scroll area with hidden scrollbar styling (`scrollbar-width: none`). Treat Classic add-on sidebar overflow as scrollable-but-visually-hidden unless later same-shape EB evidence proves a visible scrollbar. Evidence: `/private/tmp/fpb-classic-agentic-parity/C06-addons-free-gift-scrollbar/eb-initial-addon-scrollbar-probe.json` and `/private/tmp/fpb-classic-agentic-parity/C06-addons-free-gift-scrollbar/eb-addon-step-after-nav-and-select.json`.
