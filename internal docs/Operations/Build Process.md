...
If `graphify-out/` was previously scanned, old generated-output nodes can stay
in `graph.json` because code-only rebuilds preserve semantic/document nodes.
Prune nodes whose `source_file` starts with `graphify-out/` before rebuilding.
